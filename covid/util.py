...
def run_place(data, 
    ...
    if hierarchical is False:
         ...
         places = ["NY","MA"]#,"RI"]
         cov_dat,_ = load_state_Xy(which=places)

         place_data_total = onp.zeros((len(places),T,2))
         cov_dat_total = onp.zeros((len(places),T))
         ...
         for place in places:
              ...
              mask = (cov_dat.loc[place]['date'] >= start) & (cov_dat.loc[place]['date'] <= end)

              cov_dat_total[place_idx,:] = cov_dat.loc[place].loc[mask].totalTestResultsIncrease.values
              place_idx +=1
         model = model_type(
              data = place_data_total,
              T = T,
              N = N,
              cov_dat = cov_dat_total,
