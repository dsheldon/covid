...
pdf(par,0)*y_hat[39:num_data-offset+39]  ))
        ...
        if T_future > 0:
             numpyro.sample('dz_full', NB2(np.concatenate((np.zeros(offset),d_hat)), death_dispersion))
             numpyro.sample('dy_full', NB2((case_detection*y_hat), confirmed_dispersion)) 
