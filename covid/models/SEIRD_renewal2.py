...
class SEIRD(SEIRDBase):    
    ...
    def __call__(self,
         ...
         beta = numpyro.deterministic("beta", beta0 * np.exp(.9*rw))
         ...
         def SEIR_renewal(theta,x0=None, T=100, dE0=10, N=1e6, CONV_WIDTH=32):
         ...
         #      = convolution of U pmf and V ccdf
              ...
              A_full =np.convolve(U_pmf,V_pmf,mode='full')[:CONV_WIDTH]
         ...
         #
              def scan_body(state, beta):        
                 ...
                 dE = beta/gamma * S/N * np.sum(incidence_history * A_rev)
                 new_state = (np.append(incidence_history[1:], dE), S-dE)
                 return new_state, dE
     
              incidence_history = np.append(np.zeros(CONV_WIDTH-1), x0[1])
              S = x0[0]
         ...
         # various convolutions to "project forward" incident exposures
              ...
              dI = np.convolve(np.append(x0[2],dE[1:]), U_pmf, mode='full')[:T]
              I = np.convolve(dE, A_full, mode='full')[:T]
              dH = np.convolve(death_prob*np.append(x0[4],dI[1:]), V_pmf, mode='full')[:T]  
         ...
         pd= .1#numpyro.sample('time_to_death',dist.Dirichlet(.1*np.ones(40)))
         theta = (beta, sigma, gamma,death_prob,death_rate,pd)
         if T_future == 0:
              S2, E2, I2, R2,CI, dI ,dD= SEIR_renewal(theta,T=T,x0=x0, dE0=E0, CONV_WIDTH=40)
         else:
              S2, E2, I2, R2,CI, dI ,dD= SEIR_renewal(theta,x0=x0,T=T_old+T_future, dE0=E0, CONV_WIDTH=40)
         ...
         if confirmed is not None and T_future ==0:
             with numpyro.handlers.scale(scale=1.0):
                 y = observe_nb2("dy" , np.maximum(.01,np.diff(CI)), det_prob, confirmed_dispersion, obs = confirmed)
