...
class SEIRD(SEIRDBase):    
    ...
    def __call__(self,
         ...
         def SEIR_renewal(theta, incidence_history,  T=100, N=1e6, CONV_WIDTH=32):

               # beta is only time varying here

               beta, sigma, gamma, death_prob,death_rate = theta

    # U = latent period
    # V = infectious period
               U_logp = Geometric1(1/sigma)
               ...
               A = np.convolve(U_pmf, V_pmf, mode='full')[:CONV_WIDTH]
               A_rev = A[::-1] # to facilitate convolution incide the dynamics loop
               
               A_full = np.convolve(U_pmf, V_ccdf, mode='full')[:CONV_WIDTH]
               ...
               def scan_body(state, beta):        
                   ...
                   dE = beta/gamma * S/N *np.convolve(incidence_history,A ,mode='full')[len(incidence_history)]
                   new_state = (np.append(incidence_history[1:], dE), S-dE)
                   return new_state, dE

               S = N - np.sum(incidence_history)
               ...
               I = np.convolve(dE, A_full, mode='full')[:T]
               ...
               R = N-S-E-I
               return (dI , dD, incidence_history) 
         ...
         CONV_WIDTH=40         #initial incidence history
         incidence_history_init = np.append(np.zeros(CONV_WIDTH-1), E0)
         
         # run renewal model

         dI, dD, _ =  SEIR_renewal(theta, incidence_history_init, T=T, CONV_WIDTH=40,N=N)
             # observations on actual data
         y = observe_nb2("dy",dI[1:], det_prob, confirmed_dispersion, obs = confirmed)
         #z = observe_nb2("dz", dD[1:], det_prob_d, confirmed_dispersion, obs = death)
         y = np.append(dy0,y)
         z = np.append(dz0,z)
         if T_future <0:
             # update Beta to take the last 10 using last_rw
             beta_new = np.append((beta,np.repeat(beta[-rw_use_last:].mean(),T_future)))
             theta = (beta_new, sigma, gamma,death_prob,death_rate)
 
             dI, dD, incidence_history =  SEIR_renewal(theta, incidence_history_init, T=T+T_future, CONV_WIDTH=40,N=N)
             
             y_f = observe_nb2('dy_future',dI[-T_future:],np.repeat(det_prob[-rw_use_last:].mean(),T_future),confirmed_dispersion)
             z_f = observe_nb2('dy_future',dI[-T_future:],np.repeat(det_prb_d,T_future),death_dispersion)
             y = np.append(y,y_f)
             z = np.zppend(z,z_f)                    
