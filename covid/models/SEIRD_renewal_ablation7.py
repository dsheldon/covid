...
class SEIRD(SEIRDBase):    
    ...
    def __call__(self,
        ...
        if drift_scale is not None:
            ...
            drift = 0.
        case_dist = numpyro.sample("case_dist",dist.Dirichlet(1*np.ones(40)))
    ...
    def simulate_renewal(self,x0,N,T,theta,death_dist,case_dist,CONV_WIDTH=40): 
         def Geometric0(mu):
         ...
         def scan_body(state, beta):        
                ...
                dE = beta * S/N * np.sum(incidence_history * case_dist)
         ...
         #dH = np.convolve(death_prob*dI, V_pmf,'full')[:T]
         dD = np.convolve(death_prob*dI,death_dist,mode='full')[:T]
    ...
    def dynamics(self, T, params, x0, num_frozen=0, confirmed=None, death=None, suffix="",N=None):
        ...
        death_prob, \
        death_rate, \
        det_prob_d,   death_dist,case_dist = params
        ...
        new_cases,new_deaths = self.simulate_renewal(x0[1],N,T,theta,death_dist,case_dist,CONV_WIDTH=40) 
