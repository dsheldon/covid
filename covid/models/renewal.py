...
class SEIRD(SEIRDBase):    
  def __call__(self,
    ...
    def SEIR_renewal(theta, T=100, dE0=10, N=1e6, CONV_WIDTH=32):
         ...
         A = np.convolve(U_pmf, V_pmf, mode='full')[:T]
         A_rev = A[::-1] # to facilitate convolution incide the dynamics loop
         
         A_full = np.convolve(U_pmf, V_ccdf, mode='full')[:T]
    ...
    if confirmed is not None and T_future ==0:
        with numpyro.handlers.scale(scale=1.0):
            ...
            y = observe_nb2("mean_y" , np.diff(CI), det_prob, confirmed_dispersion, obs = confirmed)

        with numpyro.handlers.scale(scale=1.0):
            z = observe_nb2("dz" , dD[1:], det_prob_d, death_dispersion, obs = death)
            z = observe_nb2("mean_z" , dD[1:], det_prob_d, death_dispersion, obs = death)
 
    if T_future > 0:
        with numpyro.handlers.scale(scale=1.0):
            y = observe_nb2("dy_future" , np.diff(CI), det_prob, confirmed_dispersion)

        with numpyro.handlers.scale(scale=1.0):
            z = observe_nb2("dz_future" , dD[1:], det_prob_d, death_dispersion)
