...
def SEIRD_dynamics(T, params, x0, obs=None, death=None, suffix=""):
    ...
    with numpyro.handlers.scale(scale_factor=0.5):
        y = observe("y" + suffix, x_diff[:,6], det_prob, det_noise_scale, obs = obs)   

    with numpyro.handlers.scale(scale_factor=2.0):
        z = observe("z" + suffix, x_diff[:,5], det_prob_d, det_noise_scale, obs = death)  
...
#                            dist.TruncatedNormal(loc = 1./I_duration_est, scale = 0.25)
    ...
    obs0, obs = (None, None) if obs is None else (obs[0], np.diff(obs))
