...
places = sorted(list(data.keys())) + ['US']
#places = ['AK', 'AL']
...
for place in places:
    ...
    weekly_df = pd.DataFrame(index=t, data=np.transpose(forecast_samples)).resample("1w",label='left').sum()
...
fname = "submission_files/incident/"+forecast_start + "-UMass-MechBayes.csv"
