...
def construct_daily_df(forecast_start,forecast_samples,target,region):
...
def get_places(region):
   if region == "US":
     ...
     ignore = ["IL-Cook","NY-Kings","NY-Queens","NY-Suffolk","NJ-Bergen","NJ-Middlesex","NJ-Essex","OK-Oklahoma","MO-Jackson","NJ-Hudson","OK-Tulsa","NJ-Union","AL-Mobile","MO-St. Charles","NJ-Mercer","NJ-Somerset","MO-Greene","OK-Cleveland","UT-Weber","MO-Clay","UT-Washington","IN-Porter","OK-Canadian"]
...
def merge_df(fips_codes,forecast,region):
     if region == "US":
          ...
          df_truth["location"] = df_truth["location"].astype(str)
     elif region =="EU":
...
def generate_forecast_df(forecast_date,region,target, target_str,target_type_str,forecast_region,num_weeks,samples_directory):
     ...
     for place in get_places(region):
         ...
         daily_df = construct_daily_df(forecast_start,forecast_samples,target,place)
